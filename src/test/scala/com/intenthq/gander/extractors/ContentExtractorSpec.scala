...
class ContentExtractorSpec extends Specification {
  ...
  def date(year: Int, month: Int, day: Int) = Some(LocalDate.of(year, month, day))

  "extractDateFromURLUnsafe" >> {
    " should extract the date from the path, if present" >> {
      ContentExtractor.extractDateFromURL("http://a.com/no/date/in/this/path") must beNone
      ContentExtractor.extractDateFromURL("http://a.com/not/every/number/1900/is/a/date") must beNone
      ContentExtractor.extractDateFromURL("http://a.com/number/2000a/plus/letters") must beNone
